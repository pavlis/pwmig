...
elog_expand (char *fmt, char *result )
{
	...
	if ( c == '%' ) {
		...
		case 'h' : {
		    ...
		    uname (&utsname) ;
		    strncpy(dst, utsname.nodename, _SYS_NAMELEN) ;
		    dst += strlen(dst) ; 
		    }
		    break ; 
...
char **argv ; 
{
    ...
    /*  This was the danq code 
    if ( pthread_mutex_init ( &Lock, PTHREAD_PROCESS_PRIVATE) )
    This sets the attributes of the mutex to default */
...
/* March 2014 (GLP):  This set of procedures has some 
very dangerous constructs.  clang on the mac is throwing a
ton of warnign errors here linked to 64bit issues.   I'm going
to ignore them for now as I am hoping this won't come up much 
for this use as I'm avoiding the elog procedures.  The main
use is in the travel time engine*/
int 
